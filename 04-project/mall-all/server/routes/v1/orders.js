...
* @Last Modified time: 2020-03-02 12:24:32
*/
	...
	if(paymentType == '10'){//支付宝
		...
		if(channel == channels.page){
			...
			res.json({
				code:0,
				data:{
					url:aliPay.pagePay(options)//返回支付链接
				}
			})
		}
		else if(channel == channels.wap){
			//支付宝手机端
			options.quitUrl = HOST_DOMAIN + '/payments/alipay/quit'
			res.json({
				code:0,
				data:{
					url:aliPay.wapPay(options)//返回支付链接
				}
			})			
		}
		else if(channel == channels.app){
			//支付宝app
			res.json({
				code:0,
				data:{
					orderInfo:aliPay.appPay(options),//返回支付所需参数 由前端app通过参数拉起支付宝app支付
...
router.post('/',(req,res)=>{
	...
	.then(user=>{
		...
		.then(result=>{
			...
			//微信小程序使用自己的地址
			if(req.body.channel == channels.wxmp){
				order.shipping = {
			    	name:req.body.name,
...
router.put('/pay',(req,res)=>{
	...
	let channel = req.body.channel || channels.page
